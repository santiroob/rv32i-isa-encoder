...
instrucción del subconjunto RV32I solicitado y produce su codificación de
...
procesable para validación automática:

```text
HEX: 0xXXXXXXXX
```

El punto de entrada obligatorio del proyecto es:
...
La herramienta soporta exactamente las 12 instrucciones requeridas por la
especificación. Los campos fijos `opcode`, `funct3` y `funct7` se tomaron del
manual oficial de la ISA RISC-V para RV32I y se corroboraron ensamblando casos
con `riscv64-unknown-elf-gcc` y leyendo la palabra resultante con
...
Para la validación oficial de este proyecto se usó GNU RISC-V toolchain con los
...
La invocación para validación automática debe mantenerse exactamente así:
